...
class TelebirrController(http.Controller):
    ...
    _cancel_url = '/payment/telebirr/cancel/'
    _webhook_url = '/payment/telebirr/webhook/'

    @http.route(
        _return_url, type='http', auth='public', methods=['GET', 'POST'], csrf=False,
        save_session=False
    )
    def telebirr_return_from_checkout(self, **pdt_data):
        ...
        """
        _logger.info("Handling redirection from Telebirr with data:\n%s", pprint.pformat(pdt_data))
        _logger.info("############################# same page ##########")
        ...
        try:
            ...
            tx_sudo._handle_notification_data('telebirr', notification_data)
    ...
    def telebirr_return_from_canceled_checkout(self, tx_ref, return_access_tkn):
        ...
        _logger.info("############################# same page ##########")
        _logger.info(
            "Handling redirection from Telebirr for cancellation of transaction with reference %s",
            tx_ref,
        )

        tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data(
            'telebirr', {'telebirr_type': tx_ref}
        )
        if not payment_utils.check_access_token(return_access_tkn, tx_ref):
            raise Forbidden()
        tx_sudo._handle_notification_data('telebirr', {})
    ...
    def _verify_pdt_notification_origin(self, pdt_data, tx_sudo):
        ...
        if 'tx' not in pdt_data:  # PDT is not enabled; PayPal directly sent the notification data.
            ...
            raise Forbidden("Telebirr: PDT are not enabled; cannot verify data origin")
            ...
            url = provider_sudo._telebirr_get_api_url()
            payload = {
                'cmd': '_notify-synch',
                'tx': pdt_data['tx'],
                'at': tx_sudo.provider_id.telebirr_pdt_token,
            }
            try:
                response = requests.post(url, data=payload, timeout=10)
                response.raise_for_status()
            except (requests.exceptions.ConnectionError, requests.exceptions.HTTPError):
                raise Forbidden("Telebirr: Encountered an error when verifying PDT origin")
            else:
                notification_data = self._parse_pdt_validation_response(response.text)
                if notification_data is None:
                    raise Forbidden("Telebirr: The PDT origin was not verified by Telebirr")
    ...
    def telebirr_webhook(self, **data):
        ...
        try:
            ...
            tx_sudo._handle_notification_data('telebirr', data)
    ...
    def _verify_webhook_notification_origin(notification_data, tx_sudo):
        ...
        _logger.info("############################# same page ##########")
        url = tx_sudo.provider_id._telebirr_get_api_url()
        ...
        try:
            ...
            if response_content != 'VERIFIED':
                _logger.warning(
                    "Telebirr did not confirm the origin of the notification with data:\n%s",
