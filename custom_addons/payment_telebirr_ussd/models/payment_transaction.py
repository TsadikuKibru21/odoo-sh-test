...
class ResPartner(models.Model):
    ...
    telebirr_phone_number=fields.Char(string='Telebirr Phone')

class PaymentTransaction(models.Model):
    _inherit = 'payment.transaction'

    # Telebirr-specific fields
    telebirr_type = fields.Char(string="Telebirr Transaction Type")
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider_code != 'telebirr':
            _logger.info("Provider code is not 'telebirr', skipping.")
            return res
        _logger.info("########################### process value ###########")
        _logger.info(processing_values)
        fresh_partner = self.env['res.partner'].browse(self.partner_id.id)

        _logger.info(fresh_partner.telebirr_phone_number)
        base_url = self.provider_id._telebirr_get_api_url()
        _logger.info("Telebirr base URL: %s", base_url)

        return_url = urls.url_join(base_url, f'{TelebirrPaymentController._return_url}')
        # webhook_url = urls.url_join(base_url, f'{TelebirrPaymentController._webhook_url}/{self.reference}')
        _logger.info("Telebirr return URL: %s", return_url)
        # _logger.info("Telebirr webhook URL: %s", webhook_url)

        telebirr_payload = {
            'phone': '251'+self.partner_id.telebirr_phone_number,
            'traceNo': self.reference,
            'amount': self.amount,
            'appId': self.provider_id.telebirr_app_id,
            'apiKey': self.provider_id.telebirr_api_key,
            "payerId":self.provider_id.telebirr_payer_id,
        }
        _logger.info("Constructed Telebirr payload: %s", telebirr_payload)

        try:
            payment_response = self.provider_id._telebirr_make_request(payload=telebirr_payload)
