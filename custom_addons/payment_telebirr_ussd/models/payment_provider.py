...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['telebirr'] = {'mode': 'unique', 'domain': [('type', '=', 'bank')]}
        return res
...
class Paymentprovider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('telebirr', "Telebirr Payment Gateway")], ondelete={'telebirr': 'set default'}
    )
    ...
    telebirr_url=fields.Char(
        string="URL",
        required_if_provider='telebirr',
    )
    telebirr_payer_id=fields.Char(
        string="Payer ID",
        required_if_provider='telebirr',
    )
    ...
    def _telebirr_get_api_url(self):
        ...
        if self.state == 'enabled':
            return self.telebirr_url
     
    def _telebirr_make_request(self, payload=None, method='POST'):
           
            _logger.info("#################### came to _telebirr_make_request #####################")
            self.ensure_one()

            url = self._telebirr_get_api_url()
            try:
                if method == 'GET':
