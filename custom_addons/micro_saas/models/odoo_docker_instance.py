...
class OdooDockerInstance(models.Model):
    ...
    storage_limit_gb= fields.Float(string="Storage Limit (GB)",default=1)
    ...
    def start_instance(self,address_values):
        ...
        first_name = address_values.get('first_name', 'default_firstname')
        last_name = address_values.get('last_name', 'default_lastname')
        # Ruta al archivo docker-compose.yml modificado
        self.add_to_log("[INFO] Path to modified docker-compose.yml file")
        container_name = f"{first_name}_{last_name}".lower()
        modified_path = self.instance_data_path + '/docker-compose.yml'
        self._modify_docker_compose_file(modified_path, container_name)
    ...
    # def test_method(self,address_values):
    #     self.add_to_log(f"[INFO] Test method called with address_values: {address_values}")
    def _modify_docker_compose_file(self, modified_path, container_name):
        """Modify docker-compose.yml to set a unique container name."""
        with open(modified_path, 'r') as file:
            docker_compose_data = file.read()

        # Replace placeholder or add container_name under the service configuration
        docker_compose_data = docker_compose_data.replace(
            "CONTAINER_NAME_PLACEHOLDER", container_name
        )
